...
app = Flask(__name__)
socketio = SocketIO(app, cors_allowed_origins="*", port=3001)
...
def process_text_queue():
    ...
    while True:
        ...
        if (
            sentence_count + curr_sentences > 3
            or len(accumulated_text) + curr_char_count > 400
        ):
            # Process current accumulated text before it exceeds the limit
            if accumulated_text:
                threading.Thread(
                    target=check_and_emit, args=(accumulated_text,), daemon=True
                ).start()
